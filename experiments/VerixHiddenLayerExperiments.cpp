...
VerixHiddenLayerExperiments::experiment_on_dataset(std::string modelPath, std::string datasetPath, std::string verifier,
    ...
    getline(file, header);
    outputFile << "datapoint" << "," << "exp_size" << "," << header << '\n';
    // Iterate over each line in the file
//    TODO: reverse order! make it random or heuristic
    std::vector<std::size_t> featureOrder;
    for (std::size_t node = featureSize; node > 0; --node) {
        featureOrder.push_back(node-1);
    }
    while (std::getline(file, line)) {
        ...
        outputFile << ",";
        outputFile  << res.explanation.size() << ",";
